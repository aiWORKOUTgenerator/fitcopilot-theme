    ...
    <section className="w-full py-20 px-4 bg-gray-900 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative">
        <div className="text-center mb-16" data-aos="fade-up">
          <span className="text-xs font-bold tracking-widest uppercase text-lime-300 mb-2 block">How It Works</span>
          <h2 className="text-4xl md:text-5xl font-bold mb-4 text-white">
            Your AI-Powered <span className="bg-gradient-to-r from-lime-300 to-emerald-400 text-transparent bg-clip-text text-gradient" data-text="Fitness Journey">Fitness Journey</span>
          </h2>
        ...
        <div className="space-y-8">
          {journeySteps.map((step, index) => (
            ...
            >
              {/* Main Step Card */}
              <div 
                className={`relative p-6 md:p-8 rounded-2xl bg-gray-800/70 backdrop-blur-lg border ${
                  expandedStep === index ? 'border-lime-300/50 shadow-lg shadow-lime-300/10' : 'border-gray-700'
                } transition-all duration-300 cursor-pointer group ${
                  step.accentColor.includes('lime') ? 'lime-glow' : 
              ...
              <div 
                ...
                className={`mt-2 rounded-2xl bg-gray-800/40 border border-gray-700 overflow-hidden transition-all duration-500 ease-in-out ${
                  expandedStep === index ? 'max-h-[600px] opacity-100 p-6' : 'max-h-0 opacity-0 p-0 border-0'
                }`}
                aria-hidden={expandedStep !== index}
              >
                <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 ${
                  expandedStep === index ? 'animate-fade-slide-up' : ''
                }`}>
                ...
                <div className={`text-center ${
                  expandedStep === index ? 'animate-fade-in' : ''
                }`}>
                  <a 
                    href={
