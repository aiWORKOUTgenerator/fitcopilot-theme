...
class FitCopilot_Training_Features_Manager extends FitCopilot_Complex_Manager {
        ...
        if (isset($_POST['fitcopilot_training_features_submit'])) {
            ...
            if (isset($_POST['settings'])) {
                try {
                    ...
                    error_log('Training Features: Current settings: ' . print_r($current_settings, true));
                    
                    // Log the incoming settings data
                    error_log('Training Features: Incoming settings: ' . print_r($_POST['settings'], true));
                    ...
                    error_log('Training Features: Merged settings: ' . print_r($merged_settings, true));
                    
                    // Validate settings before saving
                    $validation_errors = $this->validate_settings($merged_settings);
                    if (!empty($validation_errors)) {
                        $error_details = implode('<br>', array_map('esc_html', $validation_errors));
                        echo '<div class="notice notice-error is-dismissible">';
                        echo '<p><strong>Settings validation failed:</strong></p>';
                    ...
                    if ($result) {
                        // Log successful save
                        error_log('Training Features: Settings saved successfully');
                        
                        // Show success message with details
                        $updated_count = count($_POST['settings']);
                        $updated_fields = array_keys($_POST['settings']);
                        echo '<div class="notice notice-success is-dismissible">';
                        echo '<p><strong>Settings updated successfully!</strong> (' . $updated_count . ' settings changed)</p>';
                        echo '<p style="font-size: 12px; color: #666;">Updated fields: ' . implode(', ', $updated_fields) . '</p>';
                        echo '</div>';
                    } else {
                        // Get last WordPress error
                        global $wpdb;
                        ...
                        if (is_wp_error($result)) {
                            $debug_info[] = 'WordPress Error: ' . $result->get_error_message();
                        }
                        
                        if (!empty($wp_error)) {
                            $debug_info[] = 'Database Error: ' . $wp_error;
                        }
                        
                        // Check if update_option specifically failed
                        $option_name = 'fitcopilot_training_features_settings';
                        $test_result = update_option($option_name . '_test', 'test_value');
                        if (!$test_result) {
                            $debug_info[] = 'WordPress update_option() function is not working properly';
                        } else {
                            delete_option($option_name . '_test');
                            $debug_info[] = 'WordPress update_option() function works, issue is with settings data';
                        }
                        ...
                        $debug_info[] = "Memory usage: " . round($memory_usage / 1024 / 1024, 2) . "MB (limit: $memory_limit)";
                        
                        // Log the detailed error
                        error_log('Training Features Settings Save Error: ' . print_r($debug_info, true));
                        
                        // Show detailed error message
                        echo '<div class="notice notice-error is-dismissible">';
                        echo '<p><strong>Error updating settings.</strong> Detailed debugging information:</p>';
                        echo '<ul style="margin-left: 20px; font-family: monospace; font-size: 12px;">';
                        foreach ($debug_info as $info) {
                            echo '<li>' . esc_html($info) . '</li>';
                        }
                        echo '</ul>';
                        echo '<p><em>Check your server error logs for additional details.</em></p>';
                        echo '</div>';
                    }
                    
                } catch (Exception $e) {
                    // Log the exception
                    error_log('Training Features Settings Exception: ' . $e->getMessage() . "\n" . $e->getTraceAsString());
                    
                    echo '<div class="notice notice-error is-dismissible">';
                    echo '<p><strong>Settings update failed with exception:</strong></p>';
                    echo '<p style="font-family: monospace; background: #f5f5f5; padding: 10px; border-radius: 4px;">';
                    echo esc_html($e->getMessage());
                    echo '</p>';
                    echo '<p><em>Full error details have been logged to your server error log.</em></p>';
                    echo '</div>';
                }
            }
        }
    ...
    private function validate_settings($settings) {
        ...
        // Validate layout_columns
        if (isset($settings['layout_columns'])) {
            $columns = intval($settings['layout_columns']);
            if ($columns < 1 || $columns > 4) {
                $errors[] = "Layout columns must be between 1 and 4 (received: $columns)";
            }
        }
        
        // Validate max_display_count
        if (isset($settings['max_display_count'])) {
            $max_count = intval($settings['max_display_count']);
            if ($max_count < -1 || $max_count > 100) {
                $errors[] = "Max display count must be -1 (all) or 0-100 (received: $max_count)";
            }
        }
        
        // Validate URLs
        $url_fields = array('cta_button_url', 'cta_logo_url');
        foreach ($url_fields as $field) {
            if (isset($settings[$field]) && !empty($settings[$field])) {
                if (!filter_var($settings[$field], FILTER_VALIDATE_URL)) {
                    $errors[] = "Invalid URL format for $field: " . $settings[$field];
                }
            }
        }
        
        // Validate colors
        $color_fields = array('cta_background_color', 'cta_text_color');
        foreach ($color_fields as $field) {
            if (isset($settings[$field]) && !empty($settings[$field])) {
                if (!preg_match('/^#[a-fA-F0-9]{6}$/', $settings[$field])) {
                    $errors[] = "Invalid hex color format for $field: " . $settings[$field];
                }
            }
        }
        
        // Validate enum fields
        if (isset($settings['cta_icon_type'])) {
            $valid_types = array('lucide', 'custom', 'emoji');
            if (!in_array($settings['cta_icon_type'], $valid_types)) {
                $errors[] = "Invalid CTA icon type: " . $settings['cta_icon_type'] . " (must be: " . implode(', ', $valid_types) . ")";
            }
        }
        
        // Validate required text fields if CTA is enabled
        if (!empty($settings['cta_enabled'])) {
            $required_cta_fields = array('cta_title', 'cta_button_text');
            foreach ($required_cta_fields as $field) {
                if (empty($settings[$field])) {
                    $errors[] = "Field '$field' is required when CTA is enabled";
                }
            }
        }
        ...
        foreach ($settings as $key => $value) {
            if (is_array($value)) {
                $errors[] = "Setting '$key' should not be an array (received: " . print_r($value, true) . ")";
            }
        }
