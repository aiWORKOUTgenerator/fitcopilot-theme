...
class FitCopilot_Training_Calendar_Renderer {
    ...
    private function render_calendar_overview($calendar_data, $stats) {
        ...
        <div class="fitcopilot-section">
            ...
            // Simplified Modal Management (replaces complex AJAX approach)
            var SimpleEventsModal = {
                showAllEventsModal: function() {
                    // Use the events data already loaded on page (same source as widget)
                    var events = window.trainingCalendarEvents || [];
                    this.populateEventsModal(events);
                    this.showModal('#all-events-modal');
                    this.bindModalEvents();
                },
                
                populateEventsModal: function(events) {
                    ...
                    if (!events || events.length === 0) {
                        container.innerHTML = '<div style="text-align: center; padding: 40px; color: #666;">' +
                                            '<p>No events found.</p>' +
                                            '<button type="button" class="button button-primary" onclick="SimpleEventsModal.hideModal()">' +
                                            'Create First Event</button></div>';
                        this.updateEventCounts(0, 0);
                        return;
                    }
                    ...
                    events.forEach(function(event) {
                        ...
                        html += '<div class="event-row" style="background: #fff; border: 1px solid #ddd; border-radius: 6px; padding: 15px; margin-bottom: 10px;">';
                        html += '<div style="display: flex; justify-content: space-between; align-items: flex-start;">';
                        html += '<div style="flex: 1;">';
                        html += '<h4 style="margin: 0 0 8px 0; color: #1d2327;">' + event.title + '</h4>';
                        html += '<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; font-size: 13px; color: #666;">';
                        html += '<div><strong>📅 Date:</strong> ' + formattedDate + '</div>';
                        html += '<div><strong>👤 Trainer:</strong> ' + (event.trainer_name || 'Not assigned') + '</div>';
                        html += '<div><strong>📍 Location:</strong> ' + (event.location || 'Not specified') + '</div>';
                        html += '<div><strong>⏱️ Duration:</strong> ' + (event.duration_minutes || 60) + ' minutes</div>';
                        html += '</div>';
                        if (event.description) {
                            html += '<p style="margin: 8px 0 0 0; font-size: 13px; color: #666;">' + event.description + '</p>';
                        }
                        html += '</div>';
                        html += '<div style="text-align: right;">';
                        html += '<div class="event-status" style="padding: 6px 12px; border-radius: 4px; font-size: 12px; font-weight: 500; color: #fff; background: ' + (event.background_color || '#10b981') + '; margin-bottom: 8px;">';
                        html += (event.booking_status || 'Available').charAt(0).toUpperCase() + (event.booking_status || 'Available').slice(1);
                        html += '</div>';
                        html += '<div style="font-size: 11px; color: #666;">ID: ' + event.id + '</div>';
                        html += '</div>';
                        html += '</div>';
                        html += '</div>';
                    });
                    ...
                    this.updateEventCounts(events.length, events.length);
                },
                
                updateEventCounts: function(total, showing) {
                    document.getElementById('total-events-count').textContent = total;
                    document.getElementById('filtered-events-count').textContent = showing;
                },
                ...
                hideModal: function() {
                    var modal = document.getElementById('all-events-modal');
                    if (modal) {
                ...
                bindModalEvents: function() {
                    // Close modal events
                    var closeButtons = document.querySelectorAll('#all-events-modal .modal-close');
                    closeButtons.forEach(function(button) {
                        button.onclick = function() {
                            SimpleEventsModal.hideModal();
                        };
                    });
                    
                    // Close on overlay click
                    var overlay = document.querySelector('#all-events-modal .modal-overlay');
                    if (overlay) {
                ...
                bindFilterEvents: function() {
                    ...
                    var applyFilters = function() {
                        ...
                        var filtered = events.filter(function(event) {
                            var trainerMatch = !filterTrainer.value || event.trainer_id == filterTrainer.value;
                            var statusMatch = !filterStatus.value || event.booking_status == filterStatus.value;
                            var typeMatch = !filterType.value || event.event_type == filterType.value;
                            return trainerMatch && statusMatch && typeMatch;
                        });
                        
                        SimpleEventsModal.populateEventsModal(filtered);
                        SimpleEventsModal.updateEventCounts(events.length, filtered.length);
                    };
                    
                    if (filterTrainer) filterTrainer.onchange = applyFilters;
                    if (filterStatus) filterStatus.onchange = applyFilters;
                    if (filterType) filterType.onchange = applyFilters;
                }
            };
            
            // Override the TrainingCalendarAdmin showAllEventsModal method
            window.addEventListener('load', function() {
                if (typeof TrainingCalendarAdmin !== 'undefined') {
                    TrainingCalendarAdmin.showAllEventsModal = function() {
                        SimpleEventsModal.showAllEventsModal();
                    };
                }
            });
            </script>
